...
const Action: React.FC<any> = ({ link, linkText }: { link: string; linkText: string }) => {
  return (
...
storiesOf('FeatureBlock', module).add('Default', () => {
  const Icon = ArrowLeft;
  const title = text('Title', 'Feature');
  const copy = text(
    'Copy',
    'Exercitation nostrud proident reprehenderit adipisicing sint aliqua labore veniam irure commodo et dolore velit.',
  );
